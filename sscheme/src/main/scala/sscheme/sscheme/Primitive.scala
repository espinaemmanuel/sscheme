...
abstract class Primitive(val op : String, val minArgs : ArgumentNumber, val maxArgs : ArgumentNumber) extends Procedure {
    ...
    override def apply(interpreter: SScheme, args : Pair) : Value = {
      ...
      doApply(interpreter, args)
    }
    
    def doApply(interpreter: SScheme, args : Pair) : Value

}

class NumberReduce(op : String, fn: (vNumber, vNumber) => vNumber) extends Primitive(op, 0, UnboundedArguments){
    override def doApply(interpreter: SScheme, args : Pair) : Value = tail(args).foldLeft(toNumber(first(args)))((acum, v)=>fn(acum, v))
}
...
object Minus extends Primitive("-", 1, UnboundedArguments) { 
  override def doApply(interpreter: SScheme, args : Pair) : Value = args.size match {
...
class BooleanPrimitive(op : String, fn : (vNumber, vNumber) => Boolean) extends  Primitive(op, 2, UnboundedArguments){
  override def doApply(interpreter: SScheme, args : Pair) : Value = {
...
object Abs  extends Primitive("abs", 1, 1){
    override def doApply(interpreter: SScheme, args : Pair) : Value = Math.abs(toNumber(first(args)).value)
}